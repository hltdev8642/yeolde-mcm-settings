...
; ============================================================================
; YeOlde: Per-Mod Backup/Restore (Feature 6)
; ============================================================================

; Backup a single mod's settings
function BackupSingleMod(string a_modName, yeolde_mcm_settings settings_mod)
	Log("BackupSingleMod() -> " + a_modName)
	_backup_mod = settings_mod

	int modIndex = _allNames.Find(a_modName)
	if (modIndex < 0 || _allMods[modIndex] == none)
		ShowBackupInfoMsg("Mod '" + a_modName + "' not found.")
		return
	endif
	...
	ShowBackupInfoMsg("Backing up '" + a_modName + "'...")
	int result = _allMods[modIndex].BackupAllPagesOptions(patcher)

	if (result == 0)
		ShowBackupInfoMsg("Mod '" + a_modName + "' backed up successfully.")
		ShowBackupModStatus(a_modName, "success", 0)
	else
		ShowBackupInfoMsg("Mod '" + a_modName + "' backup FAILED (error " + result + ").")
		ShowBackupModStatus(a_modName, "failed", result)
	endif
endfunction

; Restore a single mod's settings from its backup file
function RestoreSingleMod(string a_modName, yeolde_mcm_settings settings_mod)
	Log("RestoreSingleMod() -> " + a_modName)
	_backup_mod = settings_mod
	...
	string modFilePath = BackupConfig.GetDefaultBackupModDirectory() + a_modName + ".json"
	int jMod = JValue.readFromFile(modFilePath)
	if (jMod == 0)
		ShowBackupInfoMsg("No backup found for '" + a_modName + "'.")
		return
	endif
	...
	ShowBackupInfoMsg("Restoring '" + a_modName + "'...")
	int result = _allMods[modIndex].RestorePages(jMod, patcher)
	JValue.release(jMod)

	if (result == 0)
		ShowBackupInfoMsg("Mod '" + a_modName + "' restored successfully.")
		ShowBackupModStatus(a_modName, "success", 0)
	else
		ShowBackupInfoMsg("Mod '" + a_modName + "' restore FAILED (error " + result + ").")
		ShowBackupModStatus(a_modName, "failed", result)
	endif
endfunction
